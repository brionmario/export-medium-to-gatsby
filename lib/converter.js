...
async function convertMediumFile(filePath, outputFolder, templatePath, export_drafts) {
  ...
  try {
    ...
    try {
      ...
      press.printItem(`An error occurred while saving the images to local directory. Blog: ${postData.titleForSlug}`, false, false, 2);
    }
...
async function gatherPostData(content, options, filePath) {
  press.printItem(`Gathering post data started.`, false, false, 0);
  ...
  try {
    ...
    press.printItem(`An error occurred while inlining Gists.`, false, false, 2);
  }
